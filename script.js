class CCGenerator {
    ...
    initializeElements() {
        ...
        this.customCvvInput = document.getElementById('customCvv');
    ...
    handleCvvInput() {
        this.customCvvInput.value = this.customCvvInput.value.replace(/\D/g, '').slice(0, 4);
    }
    ...
    handleBinInput() {
        ...
        if (bin.length >= 1) {
            ...
            if (detectedType && this.cardTypeSelect.value === 'auto') {
                this.showToast(`Bandeira detectada: ${detectedType.toUpperCase()}`, 'info');
            }
        }
    }
    ...
    handleSubmit(e) {
        ...
        const customCvv = this.customCvvInput.value.trim().replace(/\D/g, '');
        const includeExpiry = this.includeExpiryCheckbox.checked;
        const includeNames = this.includeNamesCheckbox.checked;

        if (quantity < 1 || quantity > 10000) {
            this.showToast('Quantidade deve ser entre 1 e 10000', 'error');
            return;
        }

        if (customCvv && (customCvv.length < 3 || customCvv.length > 4)) {
            this.showToast('CVV deve ter 3 ou 4 dígitos', 'error');
            return;
        }

        this.generateCards({
            bin, cardType, quantity, expMonth, expYear, 
            outputFormat, customCvv, includeExpiry, includeNames
        });
    }

    generateCards(options) {
        const { bin, cardType, quantity, expMonth, expYear, outputFormat, customCvv, includeExpiry, includeNames } = options;
        
        const submitBtn = this.form.querySelector('button[type="submit"]');
        const originalText = submitBtn.innerHTML;
        submitBtn.innerHTML = '<span class="material-icons">hourglass_empty</span>Gerando...';
        ...
        setTimeout(() => {
            ...
            if (cards.length < quantity) {
                this.showToast(`Gerados ${cards.length} únicos (limite de combinações com este BIN)`, 'info');
            }
            ...
            this.showToast(`${cards.length} cartões gerados com sucesso!`);
        }, 500);
    }
    ...
    displayResults(cards, validCount, outputFormat) {
        ...
        cards.forEach(card => {
            ...
            if (outputFormat === 'pipe') {
                const expiry = card.expiry || { month: '12', year: '2028' };
        ...
        this.statsContainer.style.display = 'inline';
